...
class MBNMSteppingAction : public G4UserSteppingAction, public G4UImessenger
{
  protected:
  ...
  public:
    MBNMSteppingAction() : fFile(NULL), fTree(NULL), fNEvents(0) { 
      ResetVars(); 
      fFileNameCmd = new G4UIcmdWithAString("/MBNM/setFileName", this);
...
class MBNMDetectorConstruction : public G4VUserDetectorConstruction, public G4UImessenger
{
  public:
...
class MBNMPrimaryGeneratorAction : public G4VUserPrimaryGeneratorAction
{
  public:
...
class MBNMRunManager : public G4RunManager, public G4UImessenger
{
  private:
  ...
  public:
    MBNMRunManager() {
      SetUserInitialization(new MBNMDetectorConstruction);

      fDirectory = new G4UIdirectory("/MBNM/");
      fDirectory->SetGuidance("Parameters for MBNM MC");

      fPhysListCmd = new G4UIcmdWithAString("/MBNM/setReferencePhysList", this);
      ...
      fSeedWithUUIDCmd = new G4UIcmdWithoutParameter("/MBNM/seedWithUUID", this);
    ...
    void SetNewValue(G4UIcommand *command, G4String newValues) {
      if(command == fPhysListCmd) {
        ...
        SetUserAction(new MBNMSteppingAction);
        SetUserAction(new MBNMPrimaryGeneratorAction);
      }
...
int main(int argc, char** argv)
{
  ...
  MBNMRunManager* runManager = new MBNMRunManager;
